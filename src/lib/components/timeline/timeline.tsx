  ...
  ({ className, active = false, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          "relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-border bg-background z-10",
  ...
  ({ className, active = false, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          "absolute top-10 left-5 h-full -translate-x-1/2 w-px bg-border",
          active && "bg-primary",
          className
        )}
        {...props}
